...
const NAV = [
  ...
  { id: 'inventory', icon: 'inventory', text: 'Inventory & Store' },
  { label: 'Back Office' },
...
async function route() {
  ...
  const titles = {
    ...
    inventory: ['Inventory & Store', 'Yarn, dyes, chemicals & trims — live stock'],
...
async function viewDashboard(view) {
  ...
  const deptLine = {
    Management: 'here is the whole factory at a glance.',
    Merchandising: 'your orders and buyers are moving — keep the pipeline tight.',
    Purchase: 'requisitions are waiting on your desk — approve and place POs.',
    'Dye House': 'the batch board is live — keep shades true and machines hot.',
    Shipping: 'cartons, bookings and docs — get those containers out on time.'
  }[state.user.dept] || 'all departments are under one roof.';

  const kpis =
    kpi('Active orders', d.kpis.activeOrders, 'pipeline value ' + money(d.kpis.activeValue), '#6366f1') +
    kpi('In dye house', d.kpis.inDyeing, num(d.kpis.dyeingKg) + ' kg on machines', '#8b5cf6') +
    kpi('On floors (WIP)', d.kpis.wipOrders, 'production orders running', '#ea580c') +
    kpi('Open POs', d.kpis.openPOs, money(d.kpis.openPOValue) + ' inbound', '#f59e0b') +
    kpi('In transit', d.kpis.inTransit, 'containers on the water', '#0ea5e9') +
    kpi('Receivables', money(d.kpis.receivable), d.kpis.qcPending + ' QC · ' + d.kpis.samplesOpen + ' samples open', '#16a34a') +
    kpi('Low stock alerts', d.kpis.lowStock, d.kpis.lowStock ? '<b class="warn">reorder needed</b>' : 'stock healthy', d.kpis.lowStock ? '#ef4444' : '#10b981');

  const maxStage = Math.max(1, ...d.stageCounts.map(s => s.count));
  const pipe = '<div class="pipe">' + d.stageCounts.map(s =>
    '<div class="pipe-row"><span class="pl">' + esc(s.stage) + '</span><div class="pipe-bar"><div class="pipe-fill" style="width:' + (s.count / maxStage * 100) + '%;background:' + STAGE_COLORS[s.stage] + '"></div></div><span class="pc num">' + s.count + '</span></div>'
  ).join('') + '</div>';

  const segs = d.stageCounts.filter(s => s.count).map(s => ({ color: STAGE_COLORS[s.stage], value: s.count }));
  const donut = '<div class="donut-wrap">' + donutSvg(segs) +
    '<div class="legend">' + segs.map(s => '<span class="lg"><i style="background:' + s.color + '"></i>' + d.stageCounts.find(x => STAGE_COLORS[x.stage] === s.color).stage + ' (' + s.value + ')</span>').join('') + '</div></div>';

  const maxBar = Math.max(1, ...d.shipmentsByMonth.map(b => b.count));
  const bars = '<div class="bar-chart">' + d.shipmentsByMonth.map(b =>
    '<div class="bar-col"><span class="bar-val">' + b.count + '</span><div class="bar" style="height:' + Math.max(3, b.count / maxBar * 100) + '%"></div><span class="bar-lb">' + b.label + '</span></div>'
  ).join('') + '</div>';

  const tasks =
    taskRow('purchase', '#f59e0b', d.tasks.reqsPending, 'Requisitions awaiting approval', 'Purchase desk', '#/purchase') +
    taskRow('dye', '#8b5cf6', d.tasks.qcQueue, 'Batches in QC queue', 'Dye House lab', '#/dye') +
    ...
    taskRow('purchase', '#f59e0b', d.tasks.arrivingPOs, 'POs arriving this week', 'Gate & store', '#/purchase');
  ...
  view.innerHTML =
    '<div class="card hero mb"><h2>' + hello + ', ' + esc(state.user.name.split(' ')[0]) + ' — ' + deptLine + '</h2><p>' + esc(state.user.dept) + ' console · everything below updates live as departments act.</p>' +
    '<div class="h-stats"><div class="hs"><b class="num">' + money(d.kpis.activeValue) + '</b><span>Order book</span></div><div class="hs"><b class="num">' + num(d.kpis.dyedReadyKg) + ' kg</b><span>Dyed fabric ready</span></div><div class="hs"><b class="num">' + money(d.kpis.shippedValue) + '</b><span>Shipped to date</span></div><div class="hs"><b class="num">' + num(d.kpis.dyeingKg) + ' kg</b><span>On dye machines</span></div></div></div>' +
    '<div class="kpi-row">' + kpis + '</div>' +
    '<div class="grid g-main">' +
      '<div>' +
        '<div class="card mb"><div class="card-head"><span class="card-title">Order pipeline — merchandising to delivery</span></div>' + pipe + '</div>' +
        '<div class="grid g-2">' +
          '<div class="card"><div class="card-head"><span class="card-title">Orders by stage</span></div>' + donut + '</div>' +
          '<div class="card"><div class="card-head"><span class="card-title">Shipments per month</span><span class="right mini">by ETD</span></div>' + bars + '</div>' +
        '</div>' +
      '</div>' +
      '<div>' +
        '<div class="card mb"><div class="card-head"><span class="card-title">Needs attention</span></div>' + (tasks + due || '<div class="empty">Nothing pending — rare and beautiful.</div>') + '</div>' +
        '<div class="card"><div class="card-head"><span class="card-title">Stock alerts</span><span class="right"><button class="linklike" data-go="#/inventory">Open store ' + ICONS.arrow + '</button></span></div>' +
        ...
        ).join('') : '<div class="empty">All materials above reorder levels.</div>') + '</div>' +
      '</div>' +
    '</div>' +
    '<div class="card mt"><div class="card-head"><span class="card-title">Live activity — all departments</span><span class="right"><button class="linklike" data-go="#/activity">Full thread ' + ICONS.arrow + '</button></span></div>' + d.recentActivity.map(a => feedItem(a, true)).join('') + '</div>';
