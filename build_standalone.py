...
appjs = appjs.replace(
  ...
  "'<h2 style=\"display:flex;align-items:center;gap:10px\">Sign in <span style=\"font-size:11px;letter-spacing:.5px;background:#d1fae5;color:#047857;border:1px solid #6ee7b7;padding:4px 12px;border-radius:99px\">v6 ✓ CORRECT FILE</span></h2><div class=\"sub\">One workspace for the whole company.</div>' +"
)
assert 'v6 ✓ CORRECT FILE' in appjs
...
appjs = appjs.replace('Reset demo data</button>', 'Reset demo data</button> · <b>v6</b>')
...
setTimeout(function () {
  ...
  if (r && r.querySelector(".boot")) {
    r.innerHTML = "<div style='max-width:540px;margin:14vh auto;padding:30px;font-family:Arial,sans-serif;background:#fff;border:1px solid #e2e8f0;border-radius:14px;text-align:center;box-shadow:0 4px 16px rgba(15,23,42,.06)'>"
      ...
      + "<p style='color:#64748b;font-size:14px;line-height:1.6'>The app could not start because this downloaded file is outdated.<br>Delete it and use the newest <b>knitflow-app.html</b> —<br>the correct one shows <b>v6</b> at the bottom of the sign-in card.</p>"
