...
function pub(u) { return u ? { id: u.id, name: u.name, email: u.email, role: u.role, title: u.title, dept: u.dept } : null; }
...
function docHtml(title, shipment, order, buyer, inner) {
  return '<!doctype html><html><head><meta charset="utf-8"><title>' + title + ' — ' + shipment.id + '</title>' +
    ...
    '<h1>' + COMPANY.name + '</h1><div class="co">' + COMPANY.address + '<br>Phone ' + COMPANY.phone + ' · ' + COMPANY.email + ' · ' + COMPANY.vat + '</div>' +
...
function reportShell(title, bodyInner) {
  ...
  return '<!doctype html><html><head><meta charset="utf-8"><title>' + title + '</title><style>' +
    ...
    '@media print{body{background:#fff}.sheet{box-shadow:none;margin:0}}' +
    '</style></head><body><div class="sheet"><div class="band"></div>' +
    '<h1>' + S.name + ' — ' + title + '</h1><h2>As of ' + new Date().toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' }) + ' · generated by KnitFlow OS</h2>' +
    bodyInner +
